...
Zep 的 node/edge ListInterface使用 UUID cursor 分页，
本模块封装自动翻页逻辑（含单页Retry），对Call方透明地Return完整List。
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """单页Request，Failed时指数退避Retry。仅Retry网络/IO类瞬态Error。"""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """分页GetGraphNode，最多Return max_items 条（Default 2000）。每页Request自带Retry。"""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """分页GetGraph所有边，Return完整List。每页Request自带Retry。"""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
