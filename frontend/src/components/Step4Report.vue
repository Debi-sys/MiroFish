    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - shown after Complete -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but the entire section may not be complete yet) -->
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.facts.length} entries`)
          ]),
          ...
          }, expandedFacts.value ? `Show Less ▲` : `Show All ${props.result.facts.length} Entries ▼`)
        ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? `Show Less ▲` : `Show All ${props.result.entities.length} Entities ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `Show Less ▲` : `Show All ${props.result.relations.length} Chains ▼`)
        ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.activeFacts.length} entries`)
          ]),
          ...
          }, expandedActive.value ? `Show Less ▲` : `Show All ${props.result.activeFacts.length} Entries ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.historicalFacts.length} entries`)
          ]),
          ...
          }, expandedHistorical.value ? `Show Less ▲` : `Show All ${props.result.historicalFacts.length} Entries ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该Platform未获得回复）' || t === '(该Platform未获得回复)' || t === '[None回复]' || t === '(No response from this platform)' || t === '[No response]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // First try Chinese "QuestionX:" format
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.edges.length} edges`)
          ]),
...
const activeStep = computed(() => {
  ...
  return steps[0] || { noLabel: '--', title: 'Awaiting Start', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // Remove leading second-level heading (## xxx), since the section title is already displayed in the outer layer
  ...
  // Clean <br> before paragraph start tags following block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: when single-item <ol> are separated by paragraph content, keep numbering incremental
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task is completed, scroll to top; otherwise scroll to bottom to follow latest log
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // If starting with ## or # or >, it might be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown formatting, try to return after removing thinking process
