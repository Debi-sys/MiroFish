  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (using first 20 chars of simulation requirement as title) -->
...
// CardLayout configuration - adjusted to wider ratio
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count per row to ensure centering
    ...
    (entries) => {
      entries.forEach((entry) => {
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // AnimationCompleteThen unlock, and check if there are pending state changes
          setTimeout(() => {
            isAnimating = false
...
/*  roundsNumerical progress display */
.card-progress {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* Footer roundsNumerical progress display */
...
/* ===== History playback detail modalStyle ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
