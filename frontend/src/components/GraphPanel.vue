...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition from simulating to non-simulating state, show end hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // Curvature range increases with edge count; more edges means larger curvature range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If the edge direction is opposite to the standardized direction, flip the curvature
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // Base distance 150, increases by 40 for each additional edge
    ...
    // Add gravitational pull toward center, making isolated node clusters converge to the center area
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to the connection direction, calculated by distance ratio, ensuring curves are clearly visible
    // The more edges, the larger the offset ratio relative to distance
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, increases by 5% for each additional edge
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether dragging has started (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation when actual dragging is first detected
          d._isDragging = true
